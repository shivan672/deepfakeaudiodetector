...
def extract_features(audio, sr):
    ...
    T            = mel_norm.shape[1]
    pad          = N_MELS - 40
    mfcc_padded  = np.pad(norm(mfcc[:, :T]),
                          ((0, pad), (0, 0)))
    delta_padded = np.pad(norm(mfcc_delta[:, :T]),
                          ((0, pad), (0, 0)))
    ...
    with st.spinner("Analyzing audio..."):
        ...
        if features is None:
            ...
            if is_fake:
                ...
                st.markdown(
                    ">  This audio appears to be **artificially "
                    "generated**."
                )
            else:
                ...
                st.progress(float(confidence))
                st.markdown(
                    ">  This audio appears to be a "
                    "**genuine human voice** recording."
                )
