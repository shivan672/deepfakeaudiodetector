...
TARGET_SR       = 16000
TARGET_DURATION = 2.0        # ← reduced from 4.0
TARGET_SAMPLES  = int(TARGET_SR * TARGET_DURATION)  # 32000
N_MELS          = 64         # ← reduced from 128
HOP_LENGTH      = 256        # ← increased from 128

# Expected shape per file: (64, 126, 3) → very manageable
...
def extract_features(file_path):
    try:
        ...
        def norm(x):
            std = x.std()
            if std < 1e-6:   # ← fixes division by zero
                return x - x.mean()
            return (x - x.mean()) / std
        ...
        mel_norm = norm(mel_db)       # shape: (64, T)
...
# Limit to 5000 per class to save memory
...
if test_feat is not None:
    ...
    mem_gb = (test_feat.nbytes * 10000) / 1e9
    print(f"   Est. memory for 10k files: {mem_gb:.2f} GB")
else:
    print("❌ Failed"); exit()
...
print(f"\nMemory usage:")
print(f"  X_train: {X_train.nbytes/1e9:.2f} GB — shape {X_train.shape}")
print(f"  X_val:   {X_val.nbytes/1e9:.2f} GB — shape {X_val.shape}")
print(f"  X_test:  {X_test.nbytes/1e9:.2f} GB — shape {X_test.shape}")
...
cb = [
    ...
    callbacks.ModelCheckpoint('best_lcnn.h5', monitor='val_accuracy',
...
model.save("best_lcnn_final.h5")
print("\n✅ Model saved as best_lcnn_final.h5")
